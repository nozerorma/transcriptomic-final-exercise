...
```{r gsea_libraryload, echo = FALSE, message = FALSE}
```
...
As we have clustered results and complex experiment design, we will proceed to use only our DPN results obtained in our previous DGE experiment, with an LFC threshold of 1 (results_thresholded_DPN) 
```{r input_make}
...
# NOTE THAT THIS REPORT MAY BE FOUND IN SCRIPTS DIRECTORY WHEN CREATED